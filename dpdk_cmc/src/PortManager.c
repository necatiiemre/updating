...
{
    // CMC operates exclusively on server port 2 (PCI 0000:41:00.0). The EAL
    // allowlist injected in EalInit.c guarantees DPDK only enumerates this
    // device, so it always shows up as DPDK port_id 0 — and we label it so.
    const char *manual_pci_map[] = {
        "0000:41:00.0", // DPDK port_id 0 == server port 2
    };
